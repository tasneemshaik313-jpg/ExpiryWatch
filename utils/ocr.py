...
def _prep_variants(img):
    ...
    bw = cv2.adaptiveThreshold(sharp, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 9)
...
def _targeted_date_crops(image):
    ...
    label_boxes = []
    try:
        data = pytesseract.image_to_data(clahe, config="--oem 3 --psm 11", output_type=pytesseract.Output.DICT, timeout=12)
        for i, raw in enumerate(data.get("text", [])):
            word = (raw or "").strip()
            if word and LABEL_WORDS.search(word):
                label_boxes.append((int(data["left"][i]), int(data["top"][i]), int(data["width"][i]), int(data["height"][i]), word))
    except Exception as e:
        print("OCR data error:", e)
    focused = []
    for x, y, ww, hh, word in label_boxes:
        if re.search(r"MFG|MFD|MIG", word, re.I):
            x1=max(0,x-120); x2=min(enlarged.shape[1],x+1050); y1=max(0,y-30); y2=min(enlarged.shape[0],y+330)
            ...
            sa=cv2.adaptiveThreshold(sg,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,41,11)
            stamp_text=_ocr(sa,"--oem 3 --psm 11")
            if stamp_text: results.append(stamp_text)
    for x,y,ww,hh,word in label_boxes[:8]:
        row_h=max(hh*7,120); x1=max(0,x-int(0.08*enlarged.shape[1])); x2=min(enlarged.shape[1],x+max(ww*13,int(0.78*enlarged.shape[1]))); y1=max(0,y-hh*2); y2=min(enlarged.shape[0],y+row_h*2)
        focused.append((enlarged[y1:y2,x1:x2],word))
    band_h=max(220,enlarged.shape[0]//5)
    for frac in (0.18,0.30,0.42):
        y1=int(enlarged.shape[0]*frac); y2=min(enlarged.shape[0],y1+band_h)
        if y2-y1>70:
            band=enlarged[y1:y2,:]; _,_,band_bw=_prep_variants(band)
            numeric=_ocr(band_bw,"--oem 3 --psm 6 -c tessedit_char_whitelist=0123456789/-." )
            if numeric: results.append("DATE_SCAN "+numeric)
    for crop,label in focused[:5]:
        if crop.size==0 or crop.shape[1]<250: continue
        c_gray,c_clahe,c_bw=_prep_variants(crop)
        normal=_ocr(c_clahe,"--oem 3 --psm 6")
        if normal: results.append(normal)
...
def extract_text(image_path):
    ...
    h,w=image.shape[:2]; target_w=min(max(w,1400),1900); scale=target_w/float(w); image=cv2.resize(image,None,fx=scale,fy=scale,interpolation=cv2.INTER_CUBIC)
    gray,clahe,bw=_prep_variants(image); results=[]
    for variant,config in ((clahe,"--oem 3 --psm 11"),(bw,"--oem 3 --psm 11")):
        text=_ocr(variant,config)
        if text: results.append(text)
    zh1,zh2=int(image.shape[0]*0.55),int(image.shape[0]*0.75); date_zone=image[zh1:zh2,:]; zone=cv2.resize(date_zone,None,fx=1.5,fy=1.5,interpolation=cv2.INTER_CUBIC); zone_gray=cv2.cvtColor(zone,cv2.COLOR_BGR2GRAY); zone_adapt=cv2.adaptiveThreshold(zone_gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,41,11); zone_text=_ocr(zone_adapt,"--oem 3 --psm 11")
    if zone_text: results.append(zone_text)
    results.extend(_targeted_date_crops(image))
    unique=[]
    for item in results:
        cleaned=item.strip()
        if cleaned and cleaned not in unique: unique.append(cleaned)
    return "\n".join(unique)
