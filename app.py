...
def init_db():
    ...
    """)
    try:
        conn.execute("ALTER TABLE products ADD COLUMN extra_photo TEXT")
...
def risk_score(expiry_text):
    days = days_left(expiry_text)
    if days is None:
        return {"score": None, "label": "Unknown", "level": "unknown"}
    if days < 0:
        return {"score": 100, "label": "Expired", "level": "expired"}
    if days <= 10:
        return {"score": max(80, 100 - days * 2), "label": "High Risk", "level": "high"}
    if days <= 60:
        score = 40 + int((60 - days) * 40 / 50)
        return {"score": max(40, min(79, score)), "label": "Medium Risk", "level": "medium"}
    score = max(0, 39 - min(39, (days - 60) // 10))
    return {"score": score, "label": "Low Risk", "level": "low"}
...
def due_notice(row):
    ...
    if left is None:
        return None
    if left < 0:
        message = f"{row['product_name']} expired {abs(left)} day(s) ago."
        level = "urgent"
    elif left == 0:
        ...
        level = "urgent" if left == 3 else "warning"
    else:
        return None
    return {
        "product_id": row["id"],
        ...
        "message": message,
        "level": level,
        "url": url_for("product_details", product_id=row["id"]),
    }
...
def index():
    ...
    for row in rows:
        ...
        risk = risk_score(final_expiry)
        item.update(risk_score=risk["score"], risk_label=risk["label"], risk_level=risk["level"])
...
def analyze():
    ...
    barcode = None
    saved_photo = None
    saved_extra_photo = None

    for index, file in enumerate(incoming):
        saved = save_upload(file)
        if index == 0:
            saved_photo = saved  # front product photo
        elif saved_extra_photo is None:
            saved_extra_photo = saved  # first back/extra photo
...
def add_product():
    ...
    extra_photo = request.form.get("extra_photo", "")
    ...
    conn.execute("""
        INSERT INTO products(
            user_id,product_name,category,photo,extra_photo,barcode,ocr_text,
...
def delete_product(product_id):
    conn = get_db()
    row = conn.execute(
        "SELECT photo,extra_photo FROM products WHERE id=? AND user_id=?",
    ...
    if row:
        ...
        for photo_field in ("photo", "extra_photo"):
            if row[photo_field]:
                p = BASE_DIR / "static" / row[photo_field]
                if p.exists():
                    try:
                        p.unlink()
                    except OSError:
...
def product_details(product_id):
    ...
    if not row:
        ...
        return redirect(url_for("index"))
    product = dict(row)
    final_expiry = product["expiry_date"] or product["estimated_expiry"]
    product["days_left"] = days_left(final_expiry)
    product["status"] = status_for(final_expiry)
    risk = risk_score(final_expiry)
    product.update(risk_score=risk["score"], risk_label=risk["label"], risk_level=risk["level"])
...
def settings():
    conn = get_db()
    user = conn.execute("SELECT id,name,email FROM users WHERE id=?", (current_user_id(),)).fetchone()
    conn.close()
